  ...
  class MapController < ApiController
    ...
    def custom
      ...
      if node_ids.empty?
        ...
        way_nodes = WayNode.where(:node_id => node_ids)
        way_ids = way_nodes.collect { |way_node| way_node.id[0] }
        all_ways = Way.preload(:way_nodes, :way_tags).find(way_ids)

        # 过滤路径
        ways = []
        all_ways.each do |way|
          if !way.way_tags.collect(&:k).index(params[:key]).nil? & !way.way_tags.collect(&:v).index(params[:value]).nil?
            ways.push(way)
          end
        end
    ...
    def custom3
      ...
      # 过滤获取bbox范围内的节点
      nodes = Node.bbox(bbox).where(:visible => true).limit(Settings.max_number_of_nodes + 1)
      ...
      if node_ids.empty?
        ...
        # 获取路径编号
        # way_ids = way_nodes.collect { |way_node| way_node.id[0] }
        way_ids = way_nodes.collect(&:way_id).uniq
      ...
      # 返回节点
      nodes.each do |node|
        # && !nodes_to_fetch.index(node.id).nil?
        if node.visible 
          doc.root << node.to_simple_xml_node()
        end
      end
